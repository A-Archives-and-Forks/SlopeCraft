...
namespace libSchem {
  ...
  struct litematic_info {
    litematic_info();
    std::string lite_name_utf8{"Litematic generated by SlopeCraft."};
    std::string region_name_utf8{"Software developer : TokiNoBug."};
    std::string author_utf8{"SlopeCraft"};
    std::string description_utf8{"This litematic is generated by SlopeCraft."};
    uint64_t time_created; //< Milliseconds since 1970
    uint64_t time_modified; //< Milliseconds since 1970
  };
  ...
  struct WorldEditSchem_info {
    ...
    uint64_t date; //< Milliseconds since 1970
  };
  ...
  public:
    ...
    static constexpr ele_t invalid_ele_t = std::numeric_limits<ele_t>::max();
  ...
  public:
    ...
    [[nodiscard]] std::string check_size() const noexcept;
    [[nodiscard]] static std::string check_size(int64_t x, int64_t y, int64_t z) noexcept;

    inline void set_zero() noexcept { xzy.setZero(); }

    [[nodiscard]] inline ele_t* data() noexcept { return xzy.data(); }

    [[nodiscard]] inline const ele_t* data() const noexcept { return xzy.data(); }

    void resize(int64_t x, int64_t y, int64_t z);

    [[nodiscard]] inline bool check_version_id() const noexcept {
      return MCDataVersion::is_data_version_suitable(this->MC_major_ver,
                                                     this->MC_data_ver);
    }

    [[nodiscard]] inline const auto& tensor() const noexcept { return this->xzy; }

    [[nodiscard]] inline const auto& palette() const noexcept { return this->block_id_list; }

    [[nodiscard]] inline ele_t& operator()(int64_t x, int64_t y, int64_t z) noexcept {
    ...
    [[nodiscard]] inline const ele_t& operator()(int64_t x, int64_t y,
    ...
    [[nodiscard]] inline ele_t& operator()(int64_t idx) noexcept {
      assert(idx >= 0 && idx < this->size());
      return xzy(idx);
    }

    [[nodiscard]] inline const ele_t& operator()(int64_t idx) const noexcept {
      assert(idx >= 0 && idx < this->size());
      return xzy(idx);
    }

    [[nodiscard]] inline const char* id_at(int64_t x, int64_t y, int64_t z) const noexcept {
      assert(x >= 0 && x < this->x_range());
      assert(y >= 0 && y < this->y_range());
      assert(z >= 0 && z < this->z_range());
    ...
    [[nodiscard]] inline const char* id_at(int64_t idx) const noexcept {
      assert(idx >= 0 && idx < this->size());
      return block_id_list[xzy(idx)].c_str();
    }

    [[nodiscard]] inline ele_t* begin() noexcept { return xzy.data(); }

    [[nodiscard]] inline const ele_t* begin() const noexcept { return xzy.data(); }

    [[nodiscard]] inline ele_t* end() noexcept { return xzy.data() + xzy.size(); }
    [[nodiscard]] inline const ele_t* end() const noexcept { return xzy.data() + xzy.size(); }
    ...
    [[nodiscard]] inline int64_t x_range() const noexcept { return xzy.dimension(0); }
    [[nodiscard]] inline int64_t y_range() const noexcept { return xzy.dimension(2); }
    [[nodiscard]] inline int64_t z_range() const noexcept { return xzy.dimension(1); }

    [[nodiscard]] inline size_t palette_size() const noexcept { return block_id_list.size(); }

    [[nodiscard]] inline int64_t size() const noexcept { return xzy.size(); }

    [[nodiscard]] int64_t non_zero_count() const noexcept;
    ...
    [[nodiscard]] inline MCDataVersion::MCDataVersion_t MC_version_number() const noexcept {
    ...
    [[nodiscard]] inline ::SCL_gameVersion MC_major_version_number() const noexcept {
     ...
     */
    [[nodiscard]] bool have_invalid_block(
     ...
     * \return false The schem don't contain any invalid block.
     */
    [[nodiscard]] bool have_invalid_block(int64_t* first_invalid_block_x_pos,
    ...
    [[nodiscard]] auto& entity_list() noexcept { return this->entities; }
    [[nodiscard]] auto& entity_list() const noexcept { return this->entities; }
  ...
  public:
    ...
    [[nodiscard]] std::expected<void, std::pair<SCL_errorFlag, std::string>> export_litematic(
      std::string_view filename, const litematic_info& info) const noexcept;

    [[nodiscard]] std::expected<void, std::pair<SCL_errorFlag, std::string>> export_structure(
      std::string_view filename,
      const bool is_air_structure_void) const noexcept;

    [[nodiscard]] std::expected<void, std::pair<SCL_errorFlag, std::string>> export_WESchem(
      std::string_view filename,
  ...
  private:
    ...
    [[nodiscard]] std::expected<void, std::pair<SCL_errorFlag, std::string>> pre_check(
  ...
  [[nodiscard]] constexpr int_t ceil_up_to(int_t a, uint_t b) {
    if (a % b == 0) {
      return a;
    }
    return ((a / b) + 1) * b;
