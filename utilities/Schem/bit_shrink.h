...
class mushroom_sides {
private:
  uint8_t val{0b111111};

public:
  mushroom_sides() = default;

  explicit mushroom_sides(uint8_t _) : val(_) {
  }

  enum class side_t : uint8_t { up, down, north, south, east, west };

  [[nodiscard]] inline uint8_t u6() const noexcept { return val & 0b111111; }
  ...
  public:
    explicit proxy_ref(uint8_t* __data) : data(__data) {
    }

    explicit proxy_ref(mushroom_sides* thisptr) : data(&thisptr->val) {
    }
  ...
  [[nodiscard]] inline auto up() noexcept { return proxy_ref<side_t::up>(this); }

  [[nodiscard]] inline bool up() const noexcept {
    return proxy_ref<side_t::up>::mask & this->val;
  }

  [[nodiscard]] inline auto down() noexcept { return proxy_ref<side_t::down>(this); }

  [[nodiscard]] inline bool down() const noexcept {
    return proxy_ref<side_t::down>::mask & this->val;
  }

  [[nodiscard]] inline auto north() noexcept { return proxy_ref<side_t::north>(this); }

  [[nodiscard]] inline bool north() const noexcept {
    return proxy_ref<side_t::north>::mask & this->val;
  }

  [[nodiscard]] inline auto south() noexcept { return proxy_ref<side_t::south>(this); }

  [[nodiscard]] inline bool south() const noexcept {
    return proxy_ref<side_t::south>::mask & this->val;
  }

  [[nodiscard]] inline auto east() noexcept { return proxy_ref<side_t::east>(this); }

  [[nodiscard]] inline bool east() const noexcept {
    return proxy_ref<side_t::east>::mask & this->val;
  }

  [[nodiscard]] inline auto west() noexcept { return proxy_ref<side_t::west>(this); }

  [[nodiscard]] inline bool west() const noexcept {
    return proxy_ref<side_t::west>::mask & this->val;
  }

  [[nodiscard]] std::string to_blockid(std::string_view pure_id) const noexcept {
    std::string val;
    val.reserve(512);
    val = pure_id.data();
    val.push_back('[');
  ...
  [[nodiscard]] static mushroom_sides from_block_id(std::string_view blockid) noexcept {
    std::string pure_id;
    pure_id.resize(128);
    std::vector<std::pair<std::string, std::string>> traits;
    traits.resize(6);

    mushroom_sides result;
