...
这个文件夹存储了方块列表相关，包含`block_list.json`和若干方块对应的图片（必须为PNG格式）。
...
|     version      | string\|int |    是    |          | 方块最早出现的版本。0 代表 1.12 以前，12 代表 1.12，17 代表 1.17，以此类推 |
...
Everything about blocklist is in this folder, including `block_list.json` and block images (must be PNG format).
...
|     version      | string\|int |        Yes         |               | The earlist version when block is added. 0 means earlier than 1.12, 12 means 1.12, 17 means 1.17 and so on |
